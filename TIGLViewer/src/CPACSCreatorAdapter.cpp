...
bool CPACSCreatorAdapter::isStandardized()
{
    LOG(WARNING) << "CPACSCreatorAdapter:isStandardized: Function not implemented" << std::endl;
    if(isValid()){
        return aircraftTree.isAircraftStandardized();
    }
    return false;
}

void CPACSCreatorAdapter::standardize()
{
    if (isValid()) {
       aircraftTree.standardizeAircraft();
    }
    else {
        LOG(WARNING) << "CPACSCreatorAdapter:standardize: this adapter has is AircraftTree not build. Impossible to "
